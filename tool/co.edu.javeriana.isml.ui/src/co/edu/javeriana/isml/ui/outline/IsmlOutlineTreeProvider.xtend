...
class IsmlOutlineTreeProvider extends DefaultOutlineTreeProvider {
	...
	protected def _createChildren(IOutlineNode parentNode, ViewInstance viewInstance) {
		val blockParams = viewInstance.parameters.filter(ViewBlock)
		if(!blockParams.empty) {
			for (block : blockParams) {
				createNode(parentNode, block)
			}
		}
	}
	
	protected def _createChildren(IOutlineNode parentNode, ForView f) {
		createNode(parentNode, f.body)
	}
	...
	protected def _createNode(IOutlineNode parentNode, ViewBlock block) {
	...
	protected def String _text(ViewBlock block) '''
	«block.name ?: block.eContainingFeature.name»'''

	protected def _createChildren(IOutlineNode parentNode, IfView ifvp) {
		for (element : ifvp.body?.statements) {
			createNode(parentNode, element)
		}
	}
